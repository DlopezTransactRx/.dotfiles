...
- `primary_keys`: comma-separated list of one or more column names **as they appear in SELECT output** (i.e., after aliasing), e.g. `programid`
...
- **Resource & Table names** use **TABLE_NAME**.  
- **Module name** = `<NAMESPACE>_<TABLE_NAME>` (namespace uppercased + underscore + TABLE_NAME).
...
- Event type in MERGE filter = `<namespace>-<raw_table_name>` (both lowercased).
...
- If multiple patterns match, use the first rule above.  
- Ignore SQL casts in `sql_select`; apply only these mapping rules.
...
- `column {}` blocks use multi-line style:
  ...
  ```
- Use this exact table wiring in MERGE:
  `MERGE INTO ${snowflake_table.<TABLE_NAME>.database}.${snowflake_table.<TABLE_NAME>.schema}.${snowflake_table.<TABLE_NAME>.name}`
- The stream reference must be exactly: `FROM $$$STREAM$$$ t` (three dollar signs).

---

## 🔁 MERGE Logic (ranked upsert)
- Partition by the concatenated primary key values **from the event payload**:
  - **Single PK:** `PARTITION BY DATA:eventPayload:<pk1>::varchar`
  - **Multiple PKs:** `PARTITION BY (DATA:eventPayload:<pk1>::varchar || '||' || DATA:eventPayload:<pk2>::varchar [|| ...])`
...
- `WHEN MATCHED THEN UPDATE SET` all **non-PK** columns (including `VERSION`).
- `WHEN NOT MATCHED THEN INSERT` lists **all** columns in table order (VERSION first), with corresponding `source.` values.
...
- Preserve the `sql_select` column order.
...
```hcl
resource "snowflake_table" "<NAMESPACE>_<TABLE_NAME>" 
...
# Event Type      = <namespace>-<raw_table_name>
...
2. Each `primary_keys` item must match one **SELECT output** column (by alias if present, else base name), case-insensitive.
...
raw_table_name  = "..."
primary_keys    = "colA, ..."
sql_select      = "SELECT colA, colA ... FROM <raw_table_name>;"
```
---
- IMPORTANT: On initial prompt execution echo the Input template to the user.
